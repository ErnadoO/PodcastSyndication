...
class Channel implements ChannelInterface
{
    ...
    public function asXML()
    {
        ...
        if ($this->language !== null) {
            ...
            $xml->addChild('xmlns:spotify:countryOfOrigin', $this->language);
        }
        ...
        if ($this->image !== null) {
            ...
            $itunesImage = $xml->addChild('xmlns:itunes:image');
            $itunesImage->addAttribute('href', $this->image->url);

            $googleImage = $xml->addChild('xmlns:googleplay:image');
            $googleImage->addAttribute('href', $this->image->url);
        }
        ...
        if ($this->categories !== null) {
            ...
            foreach ($this->categories as $parent => $category)
            {
                if (!is_int($parent))
                {
                    $catParent = $xml->addChild('xmlns:itunes:category');
                    $catParent->addAttribute('text', $parent);

                    foreach ($category as $subcategory)
                    {
                        $element = $catParent->addChild('xmlns:itunes:category');
                        $element->addAttribute('text', $subcategory);
                    }
                }
                else {
                    $element = $xml->addChild('xmlns:itunes:category');
                    $element->addAttribute('text', $category);
                }
            }
        }

        if ($this->explicit !== null) {
            $xml->addChild('xmlns:itunes:explicit', $this->explicit ? 'yes' : 'no');
        }

        if ($this->keywords !== null) {
            $xml->addChild('xmlns:itunes:keywords', implode(',', $this->keywords));
        }
